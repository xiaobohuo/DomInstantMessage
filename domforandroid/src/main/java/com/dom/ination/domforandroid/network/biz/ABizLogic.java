...
import com.dom.ination.domforandroid.common.setting.SettingExtra;
...
public abstract class ABizLogic implements IHttpUtility {
    ...
    private HttpConfig resetHttpConfig(HttpConfig config, Setting action) {
        if (action != null && action.getExtras().containsKey(BASE_URL)) {
            config.baseUrl = action.getExtras().get(BASE_URL).getValue().toString();
        }
        return config;
    }
